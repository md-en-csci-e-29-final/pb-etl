...
from luigi import Target, format
...
class ParquetTarget(BaseDaskTarget):
    @classmethod
    def _read(self, path, **k):
        return dd.read_parquet(path, **k)

    @classmethod
    def _write(cls, collection, path, **kwargs):
        return dd.to_parquet(collection, path, **kwargs)


class CSVTarget(BaseDaskTarget):
    @classmethod
    def _read(self, path, **k):
        return dd.read_csv(path + "/*.csv", **k)